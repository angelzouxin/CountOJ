...
class sqlUtil:
    ...
    def __init__(self, sqliteName="/Users/angelzouxin/sqlite/ZUCCOJ/zuccOJ"):
...
if __name__ == '__main__':
    ...
    # from static.xlsUtil import xlsUtil
    util = sqlUtil()

    pre = 'xls/Count_list_'
    st = datetime.datetime.strptime('2017_04_24', '%Y_%m_%d')
    oneday = datetime.timedelta(days=1)
    nowday = datetime.datetime.today()
    while st.date() < nowday.date():
        filename = pre+st.strftime('%Y_%m_%d')+'.xls'
        print(filename)
        if os.path.exists(filename):
            util.import_daily_by_xls(filename)
        st = st + oneday
